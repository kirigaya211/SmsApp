...
const SpamScreen = () => {
  ...
  useEffect(() => {
    const loadMessages = async () => {
      try {
        ...
        const spamOnly = allMessages.filter(msg => msg.classification === 'SPAM');
  ...
  useEffect(() => {
    const unsubscribe = navigation.addListener('focus', async () => {
      try {
        ...
        console.error('Error reloading SPAM messages:', error);
      }
    });
  ...
  const handleMessagePress = (message) => {
    ...
    const senderMessages = spamMessages.filter(msg => msg.address === message.address);
